...
Typed client for `settings-service` — REST operations and event wire-types,
...
- REST client + request/response types ← `settings-service/specs/settings.tsp`
  (TypeSpec → OpenAPI 3.1 → [`@hey-api/openapi-ts`](https://heyapi.dev)).
- Event payload + header types ← `settings-service/specs/settings.asyncapi.yaml`
  (AsyncAPI 3.0.0 → `@asyncapi/parser` → `json-schema-to-typescript`).

> Scaffolded by `bun run gen:sdk settings` (the reusable SDK recipe, #308). Run
...
```sh
bunx turbo run typecheck --filter=@curaos/settings-sdk
bunx turbo run test      --filter=@curaos/settings-sdk
bunx turbo run build     --filter=@curaos/settings-sdk
```
