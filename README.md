...
**A typed client for settings-service, generated from the service's TypeSpec REST contract and AsyncAPI event contract, so any consumer imports a typed REST client + event wire-types with zero hand-written transport code. Generated from the reusable mold set by @curaos/notify-sdk for the M10 SDK package class (#278-284).**

Part of the CuraOS (Care Oriented Stack) platform. A typed client for settings-service, generated from the service's TypeSpec REST contract and AsyncAPI event contract, so any consumer imports a typed REST client + event wire-types with zero hand-written transport code. Generated from the reusable mold set by @curaos/notify-sdk for the M10 SDK package class (#278-284). Domain: neutral.
...
- Generate REST operations + request/response types from settings.tsp
- Generate event payload + header wire-types from settings.asyncapi.yaml.
...
| Area | Detail |
|---|---|
| Package | `@curaos/settings-sdk` |
| Source | `backend/packages/settings-sdk` |
| Domain | `neutral` |
| Layer | `package` |
| Exposure | Source available |

- Source path: `backend/packages/settings-sdk`
