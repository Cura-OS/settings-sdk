...
// Source of truth: the OpenAPI 3.1 the settings-service emits from its TypeSpec
// contract via `bun run spec:openapi` (= `tsp compile specs/settings.tsp`).
// scripts/generate.mjs runs that compile first, so this file always exists.
const openapiInput = resolve(here, '../../services/settings-service/dist/openapi.yaml');
